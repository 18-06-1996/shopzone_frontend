    ...
    const handleSubmit = async(e)=>{
        ...
        if(firstName && email && password &&confirmPassword){
            ...
            if(password === confirmPassword){
                ...
                const fetchData = await fetch(`${process.env.REACT_APP_SERVER_DOMAIN}/api/user/signup`,{
                    method : "POST",
                    headers : {
  ...
  return (
    ...
    <div className='w-full max-w-sm bg-white m-auto flex  flex-col p-4'>
        <div className='w-20 overflow-hidden rounded-full drop-shadow-md shadow-md m-auto relative '>
            <img src={data.image ? data.image : loginSignupLogo} className='w-full h-full'/>
        ...
        <form className='w-full py-3 flex flex-col' onSubmit={handleSubmit}>
            ...
            <button type='submit' className='max-w-[150px] m-auto w-full bg-red-500 hover:bg-red-600 
           text-white text-xl font-medium text-center py-1 rounded-full mt-4 cursor-pointer '>Sign up</button>

        </form>

        <p className='text-left text-sm mt-3' >Already have account ? <Link className='text-red-500 underline' to={"/login"}>Login</Link></p>
