...
const Home = () => {
 ...
 // console.log(productData);
  ...
  const homeProductCardListVeg = productData.filter(el => el.category === "vegitables",[])
...
<h2 className='text-4xl md:text-7xl font-bold py-3'>The Fastest Delevery in <span className='text-red-600 '> Your Home</span></h2>
...
<button className='fond-bold bg-red-500 text-slate-200 px-4 py-2 rounded-md'> Order Now</button>
</div>
...
<div className='flex w-full items-center'>
    ...
    <h2 className='font-bold text-2xl text-slate-800 mb-2'>Fresh vegitables</h2>

    <div className='ml-auto flex gap-4 ' >
      <button onClick={previousProduct} className='bg-slate-400 hover:bg-slate-500 p-2 rounded text-lg'><GrPrevious/></button>
      <button onClick={nextProduct} className='bg-slate-400 hover:bg-slate-500 p-2 rounded text-lg'><GrNext/></button>
    </div>
