  ...
  return (
    <div className='bg-slate-200 p-2 flex gap-4 rounded border border-slate-300'>
        ...
        <div className="flex flex-col gap-1  ">
          ...
          <div className=' cursor-pointer text-slate-600 hover:text-red-500' onClick={()=>dispatch(deleteCartItem(id))}>
            <AiFillDelete/>
          </div> </div>
       ...
       <button className='bg-slate-300 py-1  mt-2 rounded hover:bg-slate-500 p-1' onClick={()=>dispatch(increaseQty(id))}><span><HiPlus/></span></button>
         <p className='font-semibold p-1'>{qty}</p>
       <button  onClick={()=>dispatch(decreaseQty(id))} className='bg-slate-300 py-1  mt-2 rounded hover:bg-slate-500  p-1'><span><HiMinus/></span></button>
